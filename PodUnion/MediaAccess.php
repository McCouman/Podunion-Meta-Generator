 ...
 class PodUnion_MediaAccess
{
	...
	public function __construct(array $a = array())
	{
		...
		if ( ! defined('PodUnion_SEND_TO_EDITOR_ENABLED'))
		{
			add_action('admin_footer', array($this, 'init'));

			define('PodUnion_SEND_TO_EDITOR_ENABLED', true);
		}
	}
		...
		if ($uri AND in_array($file, array('post.php', 'post-new.php')))
		{
			...
			/* <![CDATA[ */
				...
				jQuery(function($)
				{
					...
					{
						var PodUnion_insert_button_label = '';

						var PodUnion_mediafield = null;

						var PodUnion_send_to_editor_default = send_to_editor;

						send_to_editor = function(html)
						{
							clearInterval(interval);

							if (PodUnion_mediafield)
							{
								...
								PodUnion_mediafield.val(url);

								// reset insert button label
								setInsertButtonLabel(PodUnion_insert_button_label);

								PodUnion_mediafield = null;
							}
							else
							{
								PodUnion_send_to_editor_default(html);
							}
						...
						$('[class*="<?php echo $this->button_class_name; ?>"]').on('click', function()
						{
							...
							PodUnion_mediafield = $('.<?php echo $this->field_class_name; ?>-' + name, $(this).closest('.postbox'));
							...
							function iframeSetup()
							{
								...
								{
									// run once
									if ( ! PodUnion_insert_button_label.length)
									{
										PodUnion_insert_button_label = getInsertButtonLabel();
									}
