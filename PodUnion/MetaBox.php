<?php
add_action('admin_head', array('PodUnion_MetaBox', '_global_head'));

add_action('admin_footer', array('PodUnion_MetaBox', '_global_foot'));

define('PodUnion_MODE_ARRAY', 'array');

define('PodUnion_MODE_EXTRACT', 'extract');

define('PodUnion_FIELD_HINT_TEXT', 'text');

define('PodUnion_FIELD_HINT_TEXTAREA', 'textarea');

define('PodUnion_FIELD_HINT_CHECKBOX', 'checkbox');

define('PodUnion_FIELD_HINT_CHECKBOX_MULTI', 'checkbox_multi');

define('PodUnion_FIELD_HINT_RADIO', 'radio');

define('PodUnion_FIELD_HINT_SELECT', 'select');

define('PodUnion_FIELD_HINT_SELECT_MULTI', 'select_multi');

// depreciated, use PodUnion_FIELD_HINT_SELECT_MULTI instead
define('PodUnion_FIELD_HINT_SELECT_MULTIPLE', 'select_multiple');

define('PodUnion_LOCK_TOP', 'top');

define('PodUnion_LOCK_BOTTOM', 'bottom');

define('PodUnion_LOCK_BEFORE_POST_TITLE', 'before_post_title');

define('PodUnion_LOCK_AFTER_POST_TITLE', 'after_post_title');

define('PodUnion_VIEW_START_OPENED', 'opened');

define('PodUnion_VIEW_START_CLOSED', 'closed');

define('PodUnion_VIEW_ALWAYS_OPENED', 'always_opened');

class PodUnion_MetaBox
{
	/**
	 ...
	 * PodUnion_MODE_ARRAY (default) and PodUnion_MODE_EXTRACT, this option
	 * should be used when instantiating the class.
	 *
	 * @since	1.2
	 * @access	public
	 * @var		string
	 */
	var $mode = PodUnion_MODE_ARRAY;

	/**
	 * When the mode option is set to PodUnion_MODE_EXTRACT, you have to take
	...
	function PodUnion_MetaBox($arr)
	{
		...
		if (is_array($arr))
		{
			...
			if ($this->lock_on_top) $this->lock = PodUnion_LOCK_TOP;
			elseif ($this->lock_on_bottom) $this->lock = PodUnion_LOCK_BOTTOM;
	...
	{
		if (PodUnion_MODE_ARRAY == $this->mode AND $key == $this->id)
		{
	...
	function _init()
	{
		...
		if ( ! PodUnion_MetaBox::_is_post() AND ! PodUnion_MetaBox::_is_page()) return;
	...
	function _foot()
	{
		...
		)
		{
			...
			(function($){ /* not using jQuery ondomready, code runs right away in footer */
				...
				<?php if (PodUnion_LOCK_TOP == $this->lock): ?>
				<?php if ('side' == $this->context): ?>
				var id = 'PodUnion-side-top';
				if ( ! $('#'+id).length)
				{
					$('<div></div>').attr('id',id).prependTo('#side-info-column');
				}
				<?php else: ?>
				var id = 'PodUnion-content-top';
				if ( ! $('#'+id).length)
				{
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (PodUnion_LOCK_BOTTOM == $this->lock): ?>
				<?php if ('side' == $this->context): ?>
				var id = 'PodUnion-side-bottom';
				if ( ! $('#'+id).length)
				{
				...
				var id = 'PodUnion-content-bottom';
				if ( ! $('#'+id).length)
				{
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (PodUnion_LOCK_BEFORE_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				var id = 'PodUnion-content-bpt';
				if ( ! $('#'+id).length)
				{
				...
				$('#'+id).append(mb);
				<?php endif; ?>
				<?php elseif (PodUnion_LOCK_AFTER_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				var id = 'PodUnion-content-apt';
				if ( ! $('#'+id).length)
				{
				...
				<?php if (PodUnion_VIEW_START_OPENED == $this->view): ?>
				mb.removeClass('closed');
				<?php elseif (PodUnion_VIEW_START_CLOSED == $this->view): ?>
				mb.addClass('closed');
				<?php elseif (PodUnion_VIEW_ALWAYS_OPENED == $this->view): ?>
	...
	{
		$prefix = 'PodUnion_filter_' . $this->id . '_';
	...
	{
		$prefix = 'PodUnion_action_' . $this->id . '_';
		$prefix = preg_replace('/_+/', '_', $prefix);
	...
	function _is_post()
	{
		if ('post' == PodUnion_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	function _is_page()
	{
		if ('page' == PodUnion_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		$post_type = PodUnion_MetaBox::_get_current_post_type();
	...
	function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = PodUnion_MetaBox::_get_post_id();
	...
	function _global_head()
	{
		...
		jQuery(function($)
		{
			...
			$('[class*=docopy-]').click(function(e)
			{
				...
				$.PodUnion.trigger('wpa_copy', [the_clone]);
			});
	...
	function _global_foot()
	{
		...
		(function($){ /* not using jQuery ondomready, code runs right away in footer */
			...
			$.PodUnion = $('<div></div>').attr('id','PodUnion').appendTo('body');
	...
	{
		if (!$this->in_template AND $this->mode == PodUnion_MODE_EXTRACT)
		{
		...
		$hints = array
		(
			PodUnion_FIELD_HINT_CHECKBOX_MULTI,
			PodUnion_FIELD_HINT_SELECT_MULTI,
			PodUnion_FIELD_HINT_SELECT_MULTIPLE,
		);
	...
	function _save($post_id) 
	{
		...
		PodUnion_MetaBox::clean($new_data);
		...
		if ($this->mode == PodUnion_MODE_EXTRACT)
		{
			$new_fields = array();
			...
			// keep data tidy, delete values if previously using PodUnion_MODE_ARRAY
		...
		else
		{
			...
			// keep data tidy, delete values if previously using PodUnion_MODE_EXTRACT
